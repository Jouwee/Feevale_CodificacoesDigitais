...
DROP VIEW V_CHART_CLASSIF_INCIDENTES;
-----------------------------------------------------------------------------------------------------------------------------------------------
...
CREATE OR REPLACE VIEW V_CHART_RELACAO_REVISAO_INCID AS
  SELECT MODEL, SERIES, TO_CHAR(MIN(X), '999990.990')||'-'||TO_CHAR(MAX(X), '999990.990') AS X, TO_CHAR(SUM(Y) / (SELECT COUNT(*) FROM V_FICHAS I WHERE I.PERIODO = O.PERIODO), '999990.990') AS Y, PERIODO FROM (
    SELECT 'SICLA' AS MODEL, 'Percentual de tempo gasto com correções' AS SERIES, RAZAO_REVISAO AS X, NUMERO_ERROS AS Y, PERIODO FROM (
